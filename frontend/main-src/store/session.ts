...
export const createSessionStore: StateCreator<ISessionStore> = (set, get) => ({
  ...
  updateSessionsList: (sessions: Session[]) =>
    set((state) => ({ ...state, sessions: sessions.map((ele) => ({ ...defaultSession, ...ele })) })),
  updateSession: (sessionId: string, updates: Partial<Pick<Session, keyof Session>>) =>
    set((state) => ({ ...state, sessions: updatedSession(state.sessions, sessionId, updates) })),
  addSession: (session: Partial<Pick<Session, keyof Session>>) =>
    set((state) => ({
      ...state,
      sessions: [{ ...defaultSession, ...session }, ...state.sessions],
      activeSession: { ...defaultSession, ...session },
    })),
});
