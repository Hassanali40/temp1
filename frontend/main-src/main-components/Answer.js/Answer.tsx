...
export const Answer = ({
    ...
    return (
        <>
            <div className="flex flex-col items-start p-2 bg-white shadow-[0px_1px_2px_rgba(0,0,0,0.14),0px_0px_2px_rgba(0,0,0,0.12)] rounded-[5px] max-w-full" id={messageBoxId}>
                <div className="grow">
                    <ReactMarkdown
                        ...
                        <div aria-label="References">
                            <div className="w-full">
                                <div className="flex flex-row justify-start items-center">
                                    <span className="mr-2 ml-2 text-[12px] font-normal leading-[16px] flex items-center text-[#616161]" onClick={toggleIsRefAccordionOpen}>
                                        {parsedAnswer.citations.length > 1 ? parsedAnswer.citations.length + " references" : "1 reference"}
                                    </span>
                                    <FontIcon className="flex flex-row justify-center items-center mt-1 text-[#616161] text-[10px]" onClick={handleChevronClick} iconName={chevronIsExpanded ? 'ChevronDown' : 'ChevronRight'} />
                                </div>
                            </div>
                        </div>
                    )}
                ...
                {chevronIsExpanded &&
                    <div className="mt-2 flex flex-col h-full gap-1 max-w-full">
                        {parsedAnswer.citations.map((citation, idx) => {
                            return (
                                <span title={createCitationFilepath(citation, ++idx)} key={idx} onClick={() => onCitationClicked(citation)} className="ml-2 text-[12px] font-semibold leading-[16px] max-w-full text-[#115EA3] flex flex-row items-center p-[4px_6px] gap-1 border border-[#D1D1D1] rounded-[4px] hover:underline cursor-pointer">
