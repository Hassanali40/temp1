...
import { multiLingualSpeechRecognizer } from '../../src/util/SpeechToText';
...
describe('SpeechToText', () => {
  it('creates a speech recognizer with multiple languages', async () => {
    ...
    (fetch as jest.Mock).mockResolvedValue(createFetchResponse(true, response));
    ...
    expect(recognizer.properties.getProperty('SpeechServiceConnection_AutoDetectSourceLanguages')).to.equal(
      languages.join(','),
    );
  });
  ...
  it('throws an error if speech config response not ok', async () => {
    (fetch as jest.Mock).mockResolvedValue(createFetchResponse(false, {}));
